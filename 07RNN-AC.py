...
def get_data(ret,ch):
    ...
    data_list=[]
    for i in ret.index:
        data_list.append(retch[retch.date==i])    
...
def train_model(net,train_iter,valid_iter,tc,tr,vc,vr,gamma,N,loss,input_size, output_size,dropout,
    ...
    p_list='RNNPP'+str(allow_short_selling)+utility_function+str(cost_type)
...
def get_weights(i,data_list,ret,gamma,dropout,allow_short_selling,utility_function,cost_type):
    ...
    net = RNN(input_size, hidden_size,output_size,num_layers,dropout)     
    ...
    path = 'result/weightRNN'+'_'+str(gamma)+'_'+str(allow_short_selling)+'_'+str(utility_function)+'_'+str(cost_type)
