# -*- coding: utf-8 -*-
"""
Created on Sun Nov 27 22:05:40 2022
...
#%%#PCA降维
df = StandardScaler().fit_transform(ch.iloc[:,2:])
pca = PCA(n_components=0.8)  #0.8
pc=pca.fit_transform(df)  #(411424, 39)

#将PAC降维后的因子进行横截面归一化
pcdf=pd.DataFrame(pc)
pcdf.index=ch['permno']
...
data.to_csv('data/char_pca.csv') ##获得pca降维后的特征
...
ret=pd.read_csv('data/ret_clean.csv',index_col=0).astype(float)/100 #读取收益   
ret.index.name='date'
ch=pd.read_csv('data/char_pca.csv',index_col=0).astype(float) #读取特征
ch=ch.sort_values(by=['date','permno'],ascending= True)
 
#1.主结果的投资组合权重 2.卖空约束下的投资组合权重 3. 风险厌恶系数等于10的投资组合权重
para_list=[['PCA',0,5,0.005,True,'MV',False],['PCA',0,5,0.005,False,'MV',False],['PCA',0,10,0.005,True,'MV',False] ]
